...
async def main_just_tasks():
    ...
    await asyncio.gather(*tasks_main_thread)
    return
...
async def main_plus_threads():
    ...
    # if we have blocking IO (None means a thread will be used, do not use this for hard computations)
...
async def main_plus_processes():
    ...
    with ProcessPoolExecutor(max_workers=6) as ppe:
        tasks_other_processes = [the_loop.run_in_executor(ppe, hard_math, 5) for t in range(10)]
